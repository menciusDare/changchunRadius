...
export default {
  ...
  data () {
    return {
      ...
      tableDataName: [
        ...
        { label: '装置名称', type: 'normal', prop: 'name', isCenter: true, sortable: true },
        { label: '类别', type: 'normal', prop: 'category', isCenter: true, sortable: true },
        { label: '装置数量', type: 'normal', prop: 'amount', isCenter: true, sortable: true },
  ...
  methods: {
    ...
    loadingGetLedgerDetailsNuclideList () {
      myPageApi.getLedgerDetailsNuclideList().then(res => {
    ...
    init () {
      ...
      Auth.getRayDevices(params).then((res) => {
        if (res.code === 0) {
