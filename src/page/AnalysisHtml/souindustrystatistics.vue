...
export default {
  ...
  methods: {
    ...
    exportExcel () {
      ...
      axios.get('/manager/radioactiveSources/industryExport', {
        params: {
          'demoType': query
        },
    ...
    searchInfo () {
      ...
      Api.getTableInfoHY(query).then(res => {
        if (res.code === 0) {
    ...
    // },
    handleJump (row, classType) {
      localStorage.setItem('rowdistracPage', 'souindustrystatistics')
      localStorage.setItem('rowdistractCode', row.distractCode)
      if (row.industryCode !== 'undefined') {
        localStorage.setItem('rowIndustryCode', row.industryCode)
        localStorage.setItem('rowIndustryNameIND', row.industryName)
      }

      localStorage.setItem('rowROALTYPE', classType)
      localStorage.setItem('rowROALAreaCodeIND', this.belongArea)

      this.$router.push({
        path: '/homeHtml/AnalysisHtml/souRadiationQuery',
        query: {
          page: 'souindustrystatistics',
          row: row,
          classType: classType,
