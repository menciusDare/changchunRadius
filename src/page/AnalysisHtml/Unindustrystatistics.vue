    ...
    <div class="Xralayout-main">
      ...
      <div class="Xratable">
        ...
        >
          ...
          <el-table-column prop="unsealedCount" label="单位数量">
            <template slot-scope="scope">
              <span
                class="tablenum"
                @click="
                  jumpunsealedradioactive(scope.row.industryCode, searchForm, 2,scope.row.regionCode)
                "
                >{{ scope.row.unsealedCount }}</span
              ></template
            >
          </el-table-column>
        </el-table>
  ...
  methods: {
    ...
    exportecal () {
      ...
      listDownLoad.downloadFile(
        '/nonSealedRadioactiveSources/exportIndustryStatistics',
        params,
        'get',
        '.xls'
      )
    ...
    gettablelist () {
      ...
      noRegionalstatistics.untablelist(params).then((res) => {
        if (res.code === 0) {
    ...
    jumpunsealedradioactive (userId, data, juge,regionCode) {
      this.$router.push({
        path: '/homeHtml/AnalysisHtml/unsealedradioactivejump',
        query: { userId: userId, data: data, juge: juge,regionCode:regionCode }
      })
    },
    init () {
