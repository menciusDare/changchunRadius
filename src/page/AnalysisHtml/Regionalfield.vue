    ...
    <div class="Xralayout-main">
      ...
      <div class="Xratable">
        ...
        >
          ...
          <el-table-column prop="firstCount" label="甲级场所">
            <template slot-scope="scope">
              <span
                class="tablenum"
                @click="jumpunsealedradioactive(scope.row.regionCode,searchForm,1,1)"
                >{{ scope.row.firstCount }}</span
              ></template
            >
          </el-table-column>
          <el-table-column prop=" secondCount" label="乙级场所">
            <template slot-scope="scope">
              <span
                class="tablenum"
                @click="jumpunsealedradioactive(scope.row.regionCode,searchForm,2,1)"
                >{{ scope.row.secondCount }}</span
              ></template
            >
          </el-table-column>
          <el-table-column prop=" lastCount" label="丙级场所">
            <template slot-scope="scope">
              <span
                class="tablenum"
                @click="jumpunsealedradioactive(scope.row.regionCode,searchForm,3,1)"
                >{{ scope.row.lastCount }}</span
              ></template
            >
          </el-table-column>
          <el-table-column prop="nothingCount" label="无场所">
            <template slot-scope="scope">
              <span class="tablenum" @click="jumpunsealedradioactive(scope.row.regionCode,searchForm,4,1)">{{
                scope.row.nothingCount
              }}</span></template
            >
          </el-table-column>
        </el-table>
  ...
  methods: {
    ...
    exportecal () {
      ...
      listDownLoad.downloadFile(
        '/nonSealedRadioactiveSources/exportRegionMaterialLevelCount',
        params,
        'get',
        '.xls'
      )
    ...
    gettablelist () {
      ...
      noRegionalstatistics.Unsealedtablelist(params).then((res) => {
        if (res.code === 0) {
    ...
    jumpunsealedradioactive (regionCode, data, code, juge) {
      this.$router.push({
        path: '/homeHtml/AnalysisHtml/unsealedradioactivejump',
        query: { regionCode: regionCode, data: data, code: code, juge: juge}
      })
    },
    init () {
